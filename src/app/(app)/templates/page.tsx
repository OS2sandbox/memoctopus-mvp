...
export default async function TemplatesPage() {
  ...
  return (
    <div className="mx-auto max-w-[960px] px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-[var(--text)]">Skabeloner</h1>
        ...
        {templates.map((t) => (
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-2">
              <h2 className="font-semibold text-[var(--text)]">{t.name}</h2>
              {t.isDefault && <Badge variant="default">Standard</Badge>}
            </div>
            <p className="text-sm text-[var(--text-muted)] mb-4">{t.description}</p>
            <div>
              <p className="text-xs font-medium text-[var(--text-2)] mb-2">Sektioner:</p>
              <ul className="space-y-1">
                {t.structure.sections.map((s) => (
                  <li key={s.key} className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
                    <span
                      className={`inline-block h-1.5 w-1.5 rounded-full ${
                        s.required ? 'bg-[var(--accent)]' : 'bg-[var(--border-strong)]'
                      }`}
                    />
                    {s.label}
                    {!s.required && (
                      <span className="text-[var(--border-strong)]">(valgfri)</span>
                    )}
                  </li>
                ))}
              </ul>
      ...
      {templates.length === 0 && (
        <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] px-8 py-12 text-center">
          <p className="text-sm text-[var(--text-muted)]">Ingen skabeloner fundet.</p>
        </div>
      )}
    </div>
  );
