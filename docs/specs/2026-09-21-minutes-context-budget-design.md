...
5. If any call returns `finish_reason === 'length'`, throw `MinutesTruncatedError` instead of
   returning a cut-off document.
...
Each generation logs one line: `[minutes] chars=… budget=… mode=single|split parts=… rounds=… ms=…`.
